...
我们之前实现了一堆 **layer 小模块**，
可以把它们看作 “乐高积木块（lego building blocks）”，
每个 layer 模块可以组合，堆叠，构建复杂的神经网络。

---

到目前为止：

✅ 我们已经手写了好几个 layer（Linear、BatchNorm、Tanh、FlattenConsecutive …）
✅ 也实现了 container（比如 Sequential）
✅ 整个代码风格越来越像 **PyTorch 的 torch.nn**！

---

实际上：

* 我们做的就是在 **torch.tensor** 之上，
* 自己手写一个简化版的 **torch.nn**，
* 这种风格和 PyTorch 官方的 **torch.nn** 真的很像，只不过官方版更强大、更完善。

---

接下来：

✅ 既然我们已经掌握了 torch.nn 里面的模块大致怎么实现、怎么组合，
✅ 知道了 **torch.nn.Module** 的核心思路，
✅ 以后我们可以直接用官方版 torch.nn，更省事、更高效，不用手写了！

---

一句话总结：

**通过这几次手写 layer 练习，我们已经“解锁”了 torch.nn，
知道怎么用官方 torch.nn 构建神经网络，后续可以直接用官方的了！**

---

如果你需要，我也可以帮你整理一份 **torch.nn 模块核心用法小总结表**，
帮你以后写代码的时候更快上手，要不要？🌟
...
✅ 很多时间都在 **看 PyTorch 的文档**，
✅ 研究各种 layer、API，搞清楚输入 shape 是什么，输出 shape 是什么，layer 做了什么操作，能接什么输入。

❗️ 遗憾的是：

* PyTorch 的官方文档做得 **并不好**（作者吐槽）：

  * 工程做得很硬核（比如分布式计算、低层 primitives），
  * 但 API 文档往往不完整，甚至有错误或者很难理解，
  * 经常靠自己摸索和实验，不能全信文档。
...
* 需要搞清楚张量到底是 2 维 / 3 维 / 4 维，

* 某个 layer 到底要求 NCL 还是 NLC？
  （N=Batch, C=Channel, L=Length，顺序不同会出问题）

* 很多 reshape, view, permute，

* 很容易 shape 不对，报错，出 bug。

---

### 3️⃣ 使用 Jupyter Notebook 做原型

✅ 自己平时开发的时候，通常：

* 先在 **Jupyter Notebook** 里写代码，
* 快速试验，调 shape，确认功能是对的。

→ Notebook 很适合调试 shape，因为可以边写边 print，
→ 很适合 prototype 层的实现。
...
* 会把代码 **复制到正式代码仓库里**，
* 用 VS Code 编辑正式训练代码，
* 最终从 code repo 里启动训练实验。

---

### 开发流程总结：

✅ Notebook 里开发 / 验证 → 复制到 repo → VS Code 启动正式训练
...
* 要反复看文档
* 花大量时间搞定 shape
* 先在 Notebook prototype，调试 OK 后转到正式代码仓库训练

---

如果你需要，我也可以帮你总结一份“**深度学习开发流程 tips**”表，
比如 shape 常见坑、Notebook 常用技巧，帮你以后做项目更高效，要不要？🌟
...
当然！这一段是作者总结**下一步计划（going forward）**，我帮你翻译讲解：

---

接下来还有很多事情要做，主要有 3 个方向：

---

### 1️⃣ 真正实现“扩张因果卷积” (dilated causal convolution)

✅ 下一步要把当前的 WaveNet 网络，
✅ 改成真正用 **dilated causal conv layer** 实现，
✅ 目前我们实现的是 tree-like 结构，卷积版更高效。
→ 要学会怎么实现这个卷积版本。

---

### 2️⃣ 学习 residual & skip connections
...
### 3️⃣ 建立实验框架 (experimental harness)

✅ 目前训练过程是“猜一猜 + 试一试” → 这是 **不标准的做法**！

✅ 典型的 deep learning workflow 应该是：

* 有完整的 **evaluation harness**，
* 训练脚本能接很多参数（arguments），
* 可以一键发起多个实验，
* 自动收集结果，画出 **training loss + validation loss** 曲线，
* 看哪些超参数组合有效，哪些无效，
* 批量跑 **hyperparameter search**，提升效率。
...
1️⃣ 改成卷积版的 WaveNet
2️⃣ 实现 residual/skip
3️⃣ 建立实验框架
4️⃣ 继续学习 RNN、LSTM、Transformer...

---

如果你需要，我也可以帮你整理一个 **“深度学习学习路线图”**，帮你规划接下来可以学什么模块，要不要？🌟
...
**# 挑战更低 loss！你能把 WaveNet 在这个数据集上的 loss 提到多低？**

如果你有兴趣，其实可以试试看 **能不能把目前的 best loss（1.993）进一步压低**，
作者说：目前我其实没有做太多实验，应该还有不少“低垂的果子”可以摘（还有很多潜力可以挖）。
...
* 目前这个网络里，hidden channels 的分配（各层的大小）我没怎么调整，
* 可能 embedding 维度（embedding\_dim）其实是选错的，
* 说不定换个 embedding\_dim 会好很多。

---

✅ 2️⃣ 对比 MLP：

* 甚至可能把 **原来的 MLP（单 hidden layer）版本扩大**，
* 说不定也能打败目前这个 fancy WaveNet 结构！
* 如果真是这样，可能会有点尴尬 😂
...
* 你也可以仔细读读 **WaveNet 论文**，
* 看看论文里还有哪些层 / 细节我们目前没有实现，
* 比如 GLU、residual connection、skip connection，
* 你可以手动实现，看看效果能不能更好。
...
✅ 目前 best loss 1.993 其实还有很多改进空间，
✅ 你可以试试看调整架构 / 参数 / 优化器，看看能不能 **把 loss 压得更低**！

---

如果你需要，我也可以帮你写一份 **“压低 WaveNet loss 的 7 个改进方向”清单**，
以后你做实验的时候可以参考，要不要？🌟
